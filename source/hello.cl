...
__kernel void hello(__global int* image_data)
{
	...
	{
		temp = z_x * z_x - z_y * z_y + shift_x + (x - 800 / 2) * scale;
		z_y = 2 * z_x * z_y + shift_y + (y - 600 / 2) * scale;
		z_x = temp;
	}
	image_data[y * 800 + x] = get_palette_color(i);
}
